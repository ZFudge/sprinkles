...
let sprinklesInterval = setInterval(sprinkling,speed);
...
function sprinkling() {
	...
	context.fillRect(0,sprinkles.cloudThickness, canvas.width, sprinkles.clearing);

	let fillIt = '#0000'+ backgroundColor.toString();
	context.fillStyle = fillIt;
	context.fillRect(0,sprinkles.clearing, canvas.width, canvas.height);
	backgroundcolorFlex();
...
const player = {
	...
	move: function() {
		if (player.alive) {
			if (player.horizontal < 0 && player.x < 1 || player.horizontal > 0 && player.x > canvas.width - player.width) {
				player.horizontal = 0;
			}
			...
			context.fillStyle = 'black',
			context.strokeRect(player.x+1,player.y+1,player.width-1,player.height-1);	
		} else {
			player.y -= player.vertical
...
const sprinkles = {
	...
	cloudThickness: canvas.height*0.20,
	clearing: canvas.height*0.8, //320,
	drops: [], 
	active: true,
	speedrange: 1,
	dripFrequency: 0.7,
	obstacles: {
		...
		createObstacles: function() {
			for (let i=sprinkles.obstacles.amount;i>0;i--) {
				...
				sprinkles.obstacles.storage.push(
					{
						x: Math.floor(Math.random() * (canvas.width - wth)),
						y: Math.floor(canvas.height*0.20 + Math.random() * (canvas.height*0.8 - hght)),
						width: wth,
						height: hght
					}
				);
		...
		drawObstacles: function() {
			for (let obst in sprinkles.obstacles.storage) {
				context.fillRect(sprinkles.obstacles.storage[obst].x,sprinkles.obstacles.storage[obst].y,sprinkles.obstacles.storage[obst].width,sprinkles.obstacles.storage[obst].height);
			}
		}
	},
	...
	adjust: function() {
		...
		for (let drop in sprinkles.drops) {
			...
			if (sprinkles.drops[drop].y >= canvas.height || player.collision(sprinkles.drops[drop])) {
			...
			if (sprinkles.drops[drop].y + sprinkles.drops[drop].height < sprinkles.cloudThickness) {
				...
				context.fillStyle = sprinkles.drops[drop].color;
			...
			context.fillRect(sprinkles.drops[drop].x, sprinkles.drops[drop].y, sprinkles.drops[drop].width, sprinkles.drops[drop].height);
...
function pushedKey(btn) {
	...
	if (player.alive) {
		...
		if (btn.keyCode === 17) {
			...
			sprinklesInterval = setInterval(sprinkling,speed*2);
		}	
	}
	if (btn.keyCode === 84) {
		test();
...
function releasedKey(btn) {
	if (player.alive) {
		...
		if (btn.keyCode === 70 && sprinkles.active) {
				clearInterval(sprinklesInterval);
				sprinklesInterval = setInterval(sprinkling,speed);
		}
	}
}
